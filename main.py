...
class MyApp(QWidget):
    def __init__(self):
        ...
        self.pushButtonFolder.clicked.connect(self.output_answer_folder_old)
    ...
    def output_answer_folder(self):
        ...
        for filename in os.listdir(cwd_tmp):
            ...
            input_text = input_text.strip()
            input_text = input_text.replace('\n', '')
            input_text_word_profile = input_text
            input_text = text_clean(input_text)
            input_text_word_profile = text_clean(input_text_word_profile, False, True, True)
            # print(input_text)
            profile_dict = get_profiles_dict(cwd)
            output = compare_string_to_profiles(input_text, profile_dict, ngram_length, min_freq)
            output_word_profile = compare_string_to_profiles(input_text_word_profile, profile_dict, ngram_length, min_freq, True)
            self.output.append('List for: ' + filename)
            self.output.append('Sorted List: \n {0}'.format(json.dumps(output, indent=4)))
            self.output.append('List for: ' + filename)
            self.output.append('Sorted List Word Profile: \n {0}'.format(json.dumps(output_word_profile, indent=4)))

    def output_answer_folder_old(self):
        self.output.setText('')
        ngram_length = self.spinBox.value()
        min_freq = self.doubleSpinBox.value()
        cwd_tmp = os.getcwd()
...
def analyze_freq(text, number_of_chars, ignore_spaces=True, ignore_punctuation=True, find_frequency=True,
    ...
    if word_profile_analysis:
        text = text_clean(text, False, ignore_punctuation, ignore_numbers)
    else:
...
def ngram_analyzer(text, text2, ngram_length, ignore_spaces=True, ignore_punctuation=True, ignore_numbers=True,
    ...
    # print(dict1)
    # print(total)
    dict2 = analyze_freq(text2, ngram_length, ignore_spaces, ignore_punctuation, True, ignore_numbers,
                         word_profile_analysis)
    total = 0
...
def compare_string_to_profiles(string, profiles, ngram_length=2, min_frequency=0.00000, word_profile_analysis=False):
    result_dict = {}
    for profile in profiles:
        result_dict[profile] = ngram_analyzer(string, profiles.get(profile), ngram_length, True, True, True,
                                              min_frequency, word_profile_analysis)
...
if __name__ == '__main__':
    ...
    # relative = find_frequency("in", sample_oe2)
    # print(relative)
    ...
    # dict1 = analyze(read_file_to_string("OESample1.txt"), 2, True)
    # print(dict1)
    # print(read_file_to_string("OESample1.txt"))

    # ngram_analyzer(sample_oe3, sample_oe4, 2, True, True)

    # get_tree_dict(cwd)

    # test_str = sample_oe
    # profile_dict = get_profiles_dict(cwd)
    # compare_string_to_profiles(test_str, profile_dict)
    # print(profile_dict)
